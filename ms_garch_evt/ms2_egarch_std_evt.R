...
ms2.eGARCH.std.spec <- CreateSpec(variance.spec = list(model = "eGARCH"),
                                  distribution.spec = list(distribution = "sstd"),
                                  switch.spec = list(K = 2))   

models   <- list(ms2.eGARCH.std.spec)
...
ms2.eGARCH.std.fit <- vector(mode = "list", length = length(models))
...
for (i in 1:n.ots) {
  ...
  for (j in 1:length(models)) {
    ...
    if (k.update == 1 || i %% k.update == 1) {
      ...
      ms2.eGARCH.std.fit[[j]] <- FitML(spec = models[[j]], data = residuals,
                                       ctr = list(do.se = FALSE))
      cvol <- Volatility(ms2.eGARCH.std.fit[[j]])
    ...
    VaR[i, j] <- forecast(arima.fit, h = 1)$mean - gpd.VaR$quantile*predict(ms2.eGARCH.std.fit[[j]], nahead = 1L)$vol
...
names(UC.pval) <- names(CC.pval) <- names(DQ.pval) <- c("ms2.eGARCH.std")
