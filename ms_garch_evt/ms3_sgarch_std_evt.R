...
# This script computes the 1-step ahead GPD VaR for the three-regime GARCH specification and backtests the model.

# Model specifications.
ms3.sGARCH.std.spec <- CreateSpec(variance.spec = list(model = "sGARCH"),
                                  distribution.spec = list(distribution = "std"),
                                  switch.spec = list(K = 3))

models   <- list(ms3.sGARCH.std.spec)
...
ms3.sGARCH.std.fit <- vector(mode = "list", length = length(models))
...
for (i in 1:n.ots) {
  ...
  for (j in 1:length(models)) {
    ...
    if (k.update == 1 || i %% k.update == 1) {
      ...
      ms3.sGARCH.std.fit[[j]] <- FitML(spec = models[[j]], data = residuals,
                                      ctr = list(do.se = FALSE))
      cvol <- Volatility(ms3.sGARCH.std.fit[[j]])
    ...
    VaR[i, j] <- forecast(arima.fit, h = 1)$mean - gpd.VaR$quantile*predict(ms3.sGARCH.std.fit[[j]], nahead = 1L)$vol
...
names(UC.pval) <- names(CC.pval) <- names(DQ.pval) <- c("ms3.sGARCH.std")
