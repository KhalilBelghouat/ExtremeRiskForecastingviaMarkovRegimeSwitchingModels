...
sGARCH.std.spec     <- CreateSpec(variance.spec = list(model = "sGARCH"),
                                  distribution.spec = list(distribution = "std"),
                                  switch.spec = list(K = 1))                      

models <- list(sGARCH.std.spec)
...
sGARCH.std.fit <- vector(mode = "list", length = length(models))

# Iterate over out-of-sample time.
for (i in 1:n.ots) {
  y.its    <- MASI[i:(n.its + i - 1)] # in-sample data.
  y.ots[i] <- MASI[n.its + i]         # in-sample data.       
  ...
  for (j in 1:length(models)) {
    ...
    if (k.update == 1 || i %% k.update == 1) {
      ...
      sGARCH.std.fit[[j]] <- FitML(spec = models[[j]], data = residuals,
                               ctr = list(do.se = FALSE))
      cvol <- Volatility(sGARCH.std.fit[[j]])
    ...
    VaR[i, j] <- forecast(arima.fit, h = 1)$mean - gpd.VaR$quantile*predict(sGARCH.std.fit[[j]], nahead = 1L)$vol
...
names(UC.pval) <- names(CC.pval) <- names(DQ.pval) <- c("sGARCH.std")
